 ...
 * [co.anitrend.retrofit.graphql.converter.request.GraphQLRequestConverter]
 * from `:runtime`.
 *
 * Supports both the typed [GraphQLRequest] flow (legacy; codegen consumers
 * should use the backend-neutral [co.anitrend.retrofit.graphql.model.request.GraphQLOperationRequest]
 * with `GraphQLRequestConverter` instead) and the legacy [QueryContainerBuilder]
 * flow (Gson-only).
 *
 * ## Serialization
 *
 ...
 *
 * @see co.anitrend.retrofit.graphql.converter.request.GraphQLRequestConverter
 */
@Deprecated(
